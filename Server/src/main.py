...
# This ensures API keys are available when clients are initialized
from pathlib import Path
from dotenv import load_dotenv

# Load from multiple potential .env locations
_this_dir = Path(__file__).parent
load_dotenv(_this_dir.parent.parent / ".env")  # Root project .env
load_dotenv(_this_dir.parent / ".env")         # Server/.env
...
async def server_lifespan(server: FastMCP) -> AsyncIterator[dict[str, Any]]:
    ...
    logger.info("Adjoint Server starting up")
    ...
    try:
        ...
        logger.info("Adjoint Server shut down")
...
async def health_http(_: Request) -> JSONResponse:
    return JSONResponse({
        "status": "healthy",
        "timestamp": time.time(),
        "message": "Adjoint server is running"
    })
...
# Register Meshy webhook handler for 3D model generation progress updates
from routes.meshy_webhooks import register_webhook_routes
register_webhook_routes(mcp)
logger.info("Registered Meshy webhook handler at /webhooks/meshy")
